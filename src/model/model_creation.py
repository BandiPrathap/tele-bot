...
nltk.download('wordnet')  # Descarga de datos para el lematizaxdor
...
def divide_in_folds(documents, num_folds):
    folds = [[] for _ in range(num_folds)]
    for i, document in enumerate(documents):
        folds[i % num_folds].append(document)
    return folds

# Aplica esta función a cada grupo de documentos
grouped_folds = {tag: divide_in_folds(grouped_documents[tag], num_folds) for tag in grouped_documents}
